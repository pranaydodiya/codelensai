...
/**
 * Retrieves the repository tree for the repository identified by the `repoId` query parameter.
 *
 * @returns On success, a JSON payload containing `{ tree: TreeItemResponse[], branch: string }`. On validation failure, a 400 JSON error with a message. If context retrieval fails, returns the response produced by `getEditorContext`. On internal failure, a 500 JSON error `{ error: "Failed to fetch repository tree" }`.
 */
